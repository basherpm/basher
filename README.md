...
- If `$XDG_DATA_HOME` is set, and `$XDG_DATA_HOME/basher` is a directory, then `$BASHER_ROOT` will be set to `$XDG_DATA_HOME/basher` instead of the usual `$HOME/.basher`. Usually, `$XDG_DATA_HOME` is set to `~/.local/share/basher`.
